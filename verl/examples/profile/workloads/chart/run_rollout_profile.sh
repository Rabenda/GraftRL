...
# GRPO cross-branch turn1 image-token reuse (SGLang qwen2_5_vl hook).
# SGLANG_GRPO_REUSE_MODE:
#   whole_slot_reuse                 -> slot-level exact/similarity embedding reuse
#   token_or_window_partial_reuse    -> window-level partial ViT reuse for refocus slot
export SGLANG_GRPO_SIM_CACHE="${SGLANG_GRPO_SIM_CACHE:-0}"
export SGLANG_GRPO_SIM_RAW_COSINE_THRESH="${SGLANG_GRPO_SIM_RAW_COSINE_THRESH:-0.995}"
export SGLANG_GRPO_SIM_RAW_COSINE_RATIO="${SGLANG_GRPO_SIM_RAW_COSINE_RATIO:-0.90}"
export SGLANG_GRPO_REUSE_MODE="${SGLANG_GRPO_REUSE_MODE:-whole_slot_reuse}"
export SGLANG_GRPO_ENABLE_PARTIAL_VIT_REUSE="${SGLANG_GRPO_ENABLE_PARTIAL_VIT_REUSE:-0}"
export SGLANG_GRPO_PARTIAL_REUSE_THRESHOLD="${SGLANG_GRPO_PARTIAL_REUSE_THRESHOLD:-0.98}"
export SGLANG_GRPO_PARTIAL_REUSE_GRANULARITY="${SGLANG_GRPO_PARTIAL_REUSE_GRANULARITY:-window}"
