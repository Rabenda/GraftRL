#!/usr/bin/env bash
# MMDU rollout profiling with snowball multi-turn agent loop.
#
# Dataset rows are unchanged: user questions / images stay as in parquet.
# The ``mmdu_multiturn_agent`` loop lets the model generate each assistant
# reply, appends it into the live conversation, then feeds the next dataset
# user turn (preferring new-image rounds + final question). CacheBlend can
# reuse image KV across turns within a GRPO group — unlike Geo3K's synthetic
# 2-turn refocus path.
#
# Usage:
...
#
# Single-turn fallback (legacy):
#   MMDU_MULTITURN=0 bash examples/profile/workloads/mmdu/run_mmdu_profile.sh
#
# CacheBlend example:
...
#   bash examples/profile/workloads/mmdu/run_mmdu_profile.sh
#
# See README.md for offline filtering and OOM notes.
...
DATA_ROOT="${DATA_ROOT:-${VERL_ROOT}/data/mmdu_benchmark_small}"
if [[ ! -f "${DATA_ROOT}/train.parquet" || ! -f "${DATA_ROOT}/test.parquet" || "${FORCE_MMDU_DATA:-0}" == "1" ]]; then
  DATA_ROOT="${DATA_ROOT}" FORCE="${FORCE_MMDU_DATA:-0}" bash examples/profile/workloads/mmdu/prepare_data.sh
fi
...
MMDU_MULTITURN="${MMDU_MULTITURN:-1}"
MMDU_RUNTIME_TURNS="${MMDU_RUNTIME_TURNS:-4}"
...
export MMDU_RUNTIME_TURNS
export MMDU_INTERMEDIATE_MAX_NEW_TOKENS="${MMDU_INTERMEDIATE_MAX_NEW_TOKENS:-128}"
export MMDU_FINAL_MAX_NEW_TOKENS="${MMDU_FINAL_MAX_NEW_TOKENS:-512}"
...
SUFFIX="${SUFFIX:-mmdu_multiturn_bs${TRAIN_BATCH_SIZE}_n${ROLLOUT_N}_t${MMDU_RUNTIME_TURNS}}"
LOG_ROOT="${LOG_ROOT:-${VERL_ROOT}/profile_logs_mmdu_multiturn}"
...
echo "[mmdu] train=${TRAIN_FILE}"
echo "[mmdu] batch=${TRAIN_BATCH_SIZE} n=${ROLLOUT_N} steps=${TOTAL_STEPS} max_prompt=${MAX_PROMPT_LENGTH} max_response=${MAX_RESPONSE_LENGTH}"
echo "[mmdu] multiturn=${MMDU_MULTITURN} runtime_turns=${MMDU_RUNTIME_TURNS} rollout_only=${VERL_PROFILE_ROLLOUT_ONLY}"
echo "[mmdu] intermediate/final_new_tokens=${MMDU_INTERMEDIATE_MAX_NEW_TOKENS}/${MMDU_FINAL_MAX_NEW_TOKENS} gmem=${GPU_MEMORY_UTILIZATION}"

agent_args=()
if env_flag_enabled "${MMDU_MULTITURN}"; then
  max_agent_turns=$((MMDU_RUNTIME_TURNS + 2))
  agent_args+=(
    actor_rollout_ref.rollout.agent.num_workers="${AGENT_NUM_WORKERS}"
    actor_rollout_ref.rollout.agent.default_agent_loop=mmdu_multiturn_agent
    actor_rollout_ref.rollout.agent.agent_loop_config_path=examples/profile/workloads/mmdu/mmdu_multiturn_agent_loop.yaml
...
bash examples/grpo_trainer/run_qwen2_5_vl_7b_fsdp.sh \
  ...
  trainer.test_freq=-1 \
  trainer.save_freq=-1 \
  trainer.project_name='verl_mmdu_profile' \
